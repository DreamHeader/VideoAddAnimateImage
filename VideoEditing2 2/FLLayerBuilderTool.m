...
+ (UIBezierPath*)createPathForText:(NSString*)string
                        fontHeight:(CGFloat)height
                          fontName:(CFStringRef)fontName {
  ...
  for (CFIndex runIndex = 0; runIndex < CFArrayGetCount(runArray); runIndex++) {
    ...
    for (CFIndex runGlyphIndex = 0; runGlyphIndex < CTRunGetGlyphCount(run);
         runGlyphIndex++) {
